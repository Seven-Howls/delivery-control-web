...
<template>
        ...
        <div class="shadow-header  bg-white relative">
          <div class="flex  justify-center flex-col items-center overflow-auto">
            ...
            <div class="flex justify-around items-center text-cinza font-semibold w-full bg-white">
              ...
              <p class="w-1/5 text-center text-lg">{{ delivery.date }}</p>
              <p class="w-1/5 text-center text-lg">R$ {{ delivery.value }}</p>
              <p class="w-1/5 text-center text-lg">{{ delivery.status }}</p>
            </div>
            <div></div>
          </div>
        </div>
      ...
      <div class="flex justify-center">
        <div class="flex justify-between items-start w-11/12">
          <div class="bg-cinza-tabela bg-opacity-16 borda-inferior pt-1 px-2 mt-5">
            <select class="text-gray-text bg-transparent appearance-none outline-none bg-opacity-16" v-model="selectedRows" id="rows">
  ...
  .containerTabelaTitulo {
    @apply flex w-11/12 rounded-table-header justify-around text-center bg-light-blue py-2 text-white top-25 z-[1] absolute shadow-header;
  }
